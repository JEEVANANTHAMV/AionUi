...
import { FORJINN_ASSET_PROTOCOL } from '@process/extensions';
...
const isE2ETestMode = process.env.FORJINN_DESK_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.FORJINN_DESK_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[Forjinn-Desk] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[Forjinn-Desk] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[Forjinn-Desk] Creating main window...');
  ...
  console.log(`[Forjinn-Desk] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[Forjinn-Desk] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[Forjinn-Desk] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[Forjinn-Desk] Renderer did-finish-load');
      showWindow();
    });
  ...
  const disableAutoUpdater =
    process.env.FORJINN_DESK_DISABLE_AUTO_UPDATE === '1' || process.env.FORJINN_DESK_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    ...
    console.log('[Forjinn-Desk] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[Forjinn-Desk] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[Forjinn-Desk] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[Forjinn-Desk] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[Forjinn-Desk] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[Forjinn-Desk] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[Forjinn-Desk] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[Forjinn-Desk] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[Forjinn-Desk] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[Forjinn-Desk] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[Forjinn-Desk] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[Forjinn-Desk] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[Forjinn-Desk] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[Forjinn-Desk:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  protocol.handle(FORJINN_ASSET_PROTOCOL, (request) => {
  ...
  try {
    ...
    console.error('[Forjinn-Desk] Failed to restore zoom factor:', error);
...
// Register forjinn-desk:// as the default protocol client
...
// macOS: handle forjinn-desk:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[Forjinn-Desk] App initialization failed:', error);
    app.quit();
  });
...
app.on('before-quit', async () => {
  console.log('[Forjinn-Desk] before-quit');
  setIsQuitting(true);
  isExplicitQuit = true;
  destroyTray();
  ...
  const timeout = new Promise<void>((resolve) => {
    setTimeout(() => {
      console.warn('[Forjinn-Desk] Cleanup timed out after 10s, forcing quit');
      resolve();
    }, 10000);
  });
...
app.on('will-quit', () => {
  console.log('[Forjinn-Desk] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[Forjinn-Desk] quit (exitCode=${exitCode})`);
});
