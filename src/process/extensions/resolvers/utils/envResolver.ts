...
import { FORJINN_DESK_STRICT_ENV_ENV } from '../../constants';
...
export function resolveEnvTemplates(value: string, options?: EnvResolverOptions): string {
    ...
    if (envValue === undefined) {
      ...
      if (strictMode) {
        throw new UndefinedEnvVariableError(
          varName,
          `[Extensions] Strict mode: Required environment variable "${varName}" is not defined. Set the variable or disable strict mode (FORJINN_DESK_STRICT_ENV=0).`
        );
      }
  ...
  if (!strictMode && undefinedVars.length > 0) {
    console.warn(
      `[Extensions] ${undefinedVars.length} undefined environment variable(s): ${undefinedVars.join(', ')}. Enable strict mode (FORJINN_DESK_STRICT_ENV=1) to catch these errors early.`
    );
  }
