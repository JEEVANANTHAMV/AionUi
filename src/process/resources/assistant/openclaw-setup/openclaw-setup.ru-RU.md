...
**Вариант B: Запуск Forjinn-Desk WebUI в удалённом режиме**

- **Опыт**: Доступ через браузер с полным интерфейсом Forjinn-Desk
...
- **Требования к настройке**: Запуск сервиса Forjinn-Desk WebUI, доступ через браузер
...
5. **Руководство по настройке после установки** (ВАЖНО):
   ...
   - **Руководство по использованию**:
     - **Локальное использование**: «После завершения установки OpenClaw, **перезапустите Forjinn-Desk**, затем вы сможете увидеть OpenClaw в списке доступных агентов на главной странице Forjinn-Desk и начать общаться напрямую.»
...
1. **Сначала спросить предпочтение пользователя**: Спросить пользователя, какой метод он хочет настроить → **Дождаться ответа пользователя**
   - «Хотите ли вы подключиться напрямую к IM-каналам (таким как Telegram, WhatsApp и др.) или использовать удалённый режим Forjinn-Desk WebUI?»
...
4. **Вариант B: Запуск Forjinn-Desk WebUI в удалённом режиме**
   - **Обязательно использовать навык forjinn-desk-webui-setup**: Обратиться к `references/forjinn-desk-webui.md`
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к интерфейсу настроек Forjinn-Desk**:
        - **Открыть интерфейс настроек**: Чётко сообщить пользователю, как его открыть
          - «Пожалуйста, нажмите на **значок настроек** (значок шестерёнки) в левом нижнем углу Forjinn-Desk»
          - «В меню настроек нажмите на опцию \*\*'WebUI'»
          - «Войдите в интерфейс конфигурации WebUI»
        - **Шаги настройки**: Следуйте документации `forjinn-desk-webui-setup` навыка `references/forjinn-desk-webui.md` для направления пользователя:
   ...
   - **Ключевые принципы**:
     ...
     - **Не пытайтесь установить `@forjinn-desk/webui` или подобные npm-пакеты**: WebUI — это встроенная функция Forjinn-Desk, а не отдельный пакет
...
### Навык forjinn-desk-webui-setup

**Основная документация**: `references/forjinn-desk-webui.md`

**Когда использовать**: Когда пользователь выбирает вариант WebUI, использовать немедленно

**Как использовать**:

1. **Напрямую обратиться к `references/forjinn-desk-webui.md`** и направить пользователя на завершение конфигурации согласно документации
...
5. **Руководство по настройке после установки**:
   ...
   - **Руководство по использованию**:
     - Представить локальное использование (вернуться на главную страницу Forjinn-Desk)
...
3. **Выбрал WebUI**: Использовать навык `forjinn-desk-webui-setup` → Спросить потребности → Выбрать решение → Выполнить настройку → Предоставить инструкции по использованию
...
7. **Использование навыков**:
   ...
   - Настройка WebUI → **Обязательно использовать навык `forjinn-desk-webui-setup`** (напрямую обратиться к `references/forjinn-desk-webui.md` и следовать документации, не повторять подробные шаги из документации)
