...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.FORJINN_DESK_IMG_PLATFORM;
  const baseUrl = process.env.FORJINN_DESK_IMG_BASE_URL;
  const apiKey = process.env.FORJINN_DESK_IMG_API_KEY;
  const model = process.env.FORJINN_DESK_IMG_MODEL;
...
IMPORTANT: When user provides multiple images, ALWAYS pass ALL images to the image_uris parameter as an array.`,
    ...
    async ({ prompt, image_uris, workspace_dir }) => {
      ...
      const proxy = process.env.FORJINN_DESK_IMG_PROXY || undefined;
