...
import { ForjinnDeskMcpAgent } from './agents/ForjinnDeskMcpAgent';
import { CodexMcpAgent } from './agents/CodexMcpAgent';
import { OpencodeMcpAgent } from './agents/OpencodeMcpAgent';
import { ForjinnrsMcpAgent } from './agents/ForjinnrsMcpAgent';
 ...
 * - 'forjinn-desk': @office-ai/aioncli-core (Forjinn-Desk 本地管理的 Gemini 实现)
 */
export class McpService {
   ...
   * Fork Gemini (cliPath=undefined) 使用 ForjinnDeskMcpAgent
   * Native Gemini (cliPath='gemini') 使用 GeminiMcpAgent
   *
   * Get the correct MCP agent instance based on agent config.
   * Fork Gemini (cliPath=undefined) uses ForjinnDeskMcpAgent.
  ...
  private getAgentForConfig(agent: { backend: string; cliPath?: string }): IMcpProtocol | undefined {
    // Fork Gemini 使用 ForjinnDeskMcpAgent 管理 MCP 配置
    // Fork Gemini uses ForjinnDeskMcpAgent to manage MCP config
    if (agent.backend === 'gemini' && !agent.cliPath) {
      return this.agents.get('forjinn-desk');
    }
   ...
   * Fork Gemini (backend='gemini', no cliPath) uses ForjinnDeskMcpAgent.
   */
