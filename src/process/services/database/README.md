# Forjinn-Desk Database System

本文档介绍 Forjinn-Desk 的新数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{userData}/config/forjinn-desk.db`
- **图片文件**: `{userData}/data/images/`

其中 `{userData}` 为：

- macOS: `~/Library/Application Support/Forjinn-Desk/`
- Windows: `%APPDATA%/Forjinn-Desk/`
- Linux: `~/.config/Forjinn-Desk/`
...
直接复制 `forjinn-desk.db` 和 `forjinn-desk.db-wal` 文件即可。
