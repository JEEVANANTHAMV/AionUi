 ...
 * as ForjinnDesk Channels, and warns the user about the conflict.
 */
...
/**
 * Check if OpenClaw Lark channel conflicts with ForjinnDesk credentials
 */
export function detectLarkConflict(forjinnDeskAppId: string): ConflictInfo | null {
  ...
  for (const [accountName, account] of Object.entries(feishu.accounts)) {
    if (account.enabled && account.appId === forjinnDeskAppId) {
      console.warn(
        `[OpenClawConflictDetector] Lark conflict detected: OpenClaw account "${accountName}" uses same appId: ${forjinnDeskAppId}`
      );
      return {
        platform: 'lark',
        openclawEnabled: true,
        credentialMatch: true,
        openclawCredential: forjinnDeskAppId,
      };
    }
  }
...
/**
 * Check if OpenClaw Telegram channel conflicts with ForjinnDesk credentials
 */
export function detectTelegramConflict(forjinnDeskBotToken: string): ConflictInfo | null {
  const config = readOpenClawConfig();
  ...
  if (telegram.botToken === forjinnDeskBotToken) {
    console.warn(`[OpenClawConflictDetector] Telegram conflict detected: OpenClaw uses same bot token`);
    return {
      platform: 'telegram',
      openclawEnabled: true,
      credentialMatch: true,
      openclawCredential: forjinnDeskBotToken.substring(0, 20) + '...',
    };
  }
...
export function getConflictResolutionSteps(platform: 'lark' | 'telegram'): string[] {
  ...
  return [
    `Detected conflict: OpenClaw ${platformName} channel is using the same credentials as ForjinnDesk.`,
    ``,
    `This means messages are being handled by OpenClaw, not ForjinnDesk Channels.`,
    `Switching agents in ForjinnDesk will have no effect.`,
    ``,
