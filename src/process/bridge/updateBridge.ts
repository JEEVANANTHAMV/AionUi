...
const DEFAULT_REPO = 'iOfficeAI/Forjinn-Desk';
const DEFAULT_USER_AGENT = 'Forjinn-Desk';
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = process.env.FORJINN_DESK_GITHUB_REPO?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `Forjinn-Desk-update-${Date.now()}`;
};
