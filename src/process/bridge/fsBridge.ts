...
import { FORJINN_DESK_TIMESTAMP_SEPARATOR } from '@/common/config/constants';
...
export function initFsBridge(): void {
        ...
        const request = client.get(
          targetUrl,
          {
            headers: {
              ...
              Referer: 'https://github.com/iOfficeAI/Forjinn-Desk',
            },
          },
          (response) => {
  ...
  ipcBridge.fs.createTempFile.provider(async ({ fileName }) => {
    try {
      ...
      if (fileExists) {
        ...
        const tempFileName = `${name}${FORJINN_DESK_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  ipcBridge.fs.createUploadFile.provider(async ({ fileName, conversationId }) => {
    try {
      ...
      if (fileExists) {
        ...
        const ext = path.extname(safeFileName);
        const name = path.basename(safeFileName, ext);
        const newFileName = `${name}${FORJINN_DESK_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  // Skills Market: inject the forjinn-desk-skills builtin skill
  ipcBridge.fs.enableSkillsMarket.provider(async () => {
    try {
      const { getAutoSkillsDir } = await import('@process/utils/initStorage');
      const skillDir = path.join(getAutoSkillsDir(), 'forjinn-desk-skills');
  ...
  // Skills Market: remove the forjinn-desk-skills builtin skill
...
/**
 * Read the bundled SKILL.md for forjinn-desk-skills from app resources.
 *
...
async function readBundledSkillsMarketMd(): Promise<string> {
  try {
    const fallbackPath = path.join(getBuiltinSkillsCopyDir(), 'forjinn-desk-skills', 'SKILL.md');
