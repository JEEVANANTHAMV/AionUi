...
Channels 是 Forjinn-Desk 的多平台 AI 助手框架，将 Forjinn-Desk 的 AI 能力（Gemini、Claude、Codex）通过即时通讯平台暴露给远程用户。目前支持三个平台：
...
1. **ipcBridge** → Renderer 进程（更新 Forjinn-Desk 桌面界面）
2. **channelEventBus** → ChannelMessageService（更新 IM 平台消息）

这两条路径互不干扰，实现了 Forjinn-Desk 桌面端和 IM 端的同步显示。
...
4. 本地用户在 Forjinn-Desk Settings → Channels 中点击 "Approve"
...
-- source 列: 'forjinn-desk' | 'telegram' | 'lark' | 'dingtalk'
