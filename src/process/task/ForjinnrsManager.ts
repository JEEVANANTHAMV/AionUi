...
import { ForjinnrsAgent, type StdioMcpOption } from '@process/agent/forjinnrs';
import type { ForjinnrsCapabilities } from '@process/agent/forjinnrs/protocol';
...
// Forjinnrs-specific approval key — reuses same pattern as GeminiApprovalStore
type ForjinnrsApprovalKey = IApprovalKey & {
...
export class ForjinnrsApprovalStore extends BaseApprovalStore<ForjinnrsApprovalKey> {
  static createKeysFromConfirmation(action: string, commandType?: string): ForjinnrsApprovalKey[] {
    ...
    if (action === 'edit' || action === 'info' || action === 'mcp') {
      return [{ action: action as ForjinnrsApprovalKey['action'] }];
    }
    return [];
  }
}

type ForjinnrsManagerData = {
  workspace: string;
  proxy?: string;
...
export class ForjinnrsManager extends BaseAgentManager<ForjinnrsManagerData, string> {
  workspace: string;
  model: TProviderWithModel;
  readonly approvalStore = new ForjinnrsApprovalStore();
  private agent: ForjinnrsAgent | null = null;
  private agentReady: Promise<void>;
  private currentMode: string = 'default';
  private _capabilities: ForjinnrsCapabilities | null = null;
  ...
  constructor(data: ForjinnrsManagerData, model: TProviderWithModel) {
    super('forjinnrs', { ...data, model }, new IpcAgentEventEmitter(), false);
  ...
  /**
   * Determine new vs resume session, then create the ForjinnrsAgent in-process.
  ...
  override async start() {
    ...
    const agent = new ForjinnrsAgent({
      ...
      stdioMcpServers,
      onStreamEvent: (event) => this.emit('forjinnrs.message', event),
    });
  ...
  /**
   * Build the team-guide MCP stdio config for a solo forjinnrs session, or return
  ...
  > {
    ...
    if (!(await shouldInjectTeamGuideMcp('forjinnrs'))) return undefined;
    ...
    return {
      ...
      env: [
        ...base.env,
        { name: 'FORJINN_MCP_BACKEND', value: 'forjinnrs' },
        { name: 'FORJINN_MCP_CONVERSATION_ID', value: this.conversation_id },
      ],
    };
  }
  ...
  async sendMessage(data: { content: string; msg_id: string; files?: string[] }) {
    ...
    mainLog('[ForjinnrsManager]', `message sent: msg_id=${data.msg_id}`);
    if (this.agent) {
  ...
  private flushThinkingToDb(duration: number | undefined, status: 'thinking' | 'done'): void {
    ...
    addOrUpdateMessage(this.conversation_id, tMessage, 'forjinnrs');
  }
  ...
  private flushBufferedStreamText(key: string): void {
    ...
    addOrUpdateMessage(this.conversation_id, buffered.message, 'forjinnrs');
  }
  ...
  private saveContextUsage(data: unknown): void {
    ...
    void (async () => {
      try {
        ...
        if (result.success && result.data && result.data.type === 'forjinnrs') {
  ...
  init() {
    this.on('forjinnrs.message', (data) => {
      ...
      if (data.type === 'config_changed') {
        ...
        mainLog('[ForjinnrsManager]', `config_changed received (${elapsed})`, data.data);
        this._configSentAt = null;
        this._capabilities = data.data as ForjinnrsCapabilities;
        ipcBridge.conversation.responseStream.emit({
          type: 'config_changed',
          conversation_id: this.conversation_id,
          msg_id: '',
      ...
      // Log info events from forjinnrs (includes set_config/set_mode acknowledgments)
      if (data.type === 'info') {
        const elapsed = this._configSentAt ? ` (${Date.now() - this._configSentAt}ms since command)` : '';
        mainLog('[ForjinnrsManager]', `info: ${data.data}${elapsed}`);
      }
      ...
      if (data.type === 'start') {
        ...
        mainLog('[ForjinnrsManager]', `stream_start: msg_id=${data.msg_id}, TTFT=${ttft}`);
        this.status = 'running';
      ...
      if (processedData.type === 'finish') {
        ...
        mainLog('[ForjinnrsManager]', `stream_end: msg_id=${processedData.msg_id}, total=${total}`, processedData.data);
  ...
  private async handleTurnEnd(): Promise<void> {
    ...
    try {
      ...
      await processCronInMessage(this.conversation_id, 'forjinnrs', cronMessage, (sysMsg) => {
      ...
      mainError('[ForjinnrsManager]', 'Cron command processing failed', error);
    }
  }

  getCapabilities(): ForjinnrsCapabilities | null {
  ...
  async setMode(mode: string): Promise<{ success: boolean; data?: { mode: string } }> {
    ...
    if (this.agent) {
      ...
      mainLog('[ForjinnrsManager]', `set_mode sent: mode=${mode}`);
  ...
  private async saveSessionMode(mode: string): Promise<void> {
    try {
      ...
      mainError('[ForjinnrsManager]', 'Failed to save session mode', error);
    }
  }
  ...
  confirm(id: string, callId: string, data: string) {
    ...
    if (data === ToolConfirmationOutcome.ProceedAlways) {
      ...
      if (confirmation?.action) {
        const keys = ForjinnrsApprovalStore.createKeysFromConfirmation(confirmation.action, confirmation.commandType);
