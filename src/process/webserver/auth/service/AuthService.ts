...
const DUMMY_BCRYPT_PASSWORD = 'forjinn-desk-auth-dummy-password';
...
export class AuthService {
   ...
   *
   * 复用 Web 登录 token (audience: forjinn-desk-webui)
   *
  ...
  public static async verifyWebSocketToken(token: string): Promise<TokenPayload | null> {
    try {
      ...
      const decoded = jwt.verify(token, await this.getJwtSecret(), {
        issuer: 'forjinn-desk',
        audience: 'forjinn-desk-webui', // 使用与 Web 登录相同的 audience
      }) as RawTokenPayload;
