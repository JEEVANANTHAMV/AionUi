...
export const CSRF_COOKIE_NAME = 'forjinn-desk-csrf-token';
...
export const AUTH_CONFIG = {
  ...
  COOKIE: {
    ...
    NAME: 'forjinn-desk-session' as const,
    OPTIONS: {
 ...
 *
 * 信号来源（按优先级）：
 * 1. FORJINN_DESK_HTTPS=true 或 NODE_ENV=production + HTTPS=true（显式开关）
 ...
 *
 * Signals (by priority):
 * 1. FORJINN_DESK_HTTPS=true / NODE_ENV=production + HTTPS=true (explicit opt-in)
