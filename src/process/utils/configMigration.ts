...
export function getElectronConfigCandidatePaths(): string[] {
  ...
  if (process.platform === 'darwin') {
    return [
      path.join(home, '.forjinn-desk-config', 'forjinn-desk-config.txt'),
      path.join(home, '.forjinn-desk-config-dev', 'forjinn-desk-config.txt'),
    ];
  }
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(home, 'AppData', 'Roaming');
    return [
      path.join(appData, 'Forjinn-Desk', 'config', 'forjinn-desk-config.txt'),
      path.join(appData, 'Forjinn-Desk-Dev', 'config', 'forjinn-desk-config.txt'),
    ];
  }
  // Linux and other platforms
  return [
    path.join(home, '.config', 'Forjinn-Desk', 'config', 'forjinn-desk-config.txt'),
    path.join(home, '.config', 'Forjinn-Desk-Dev', 'config', 'forjinn-desk-config.txt'),
  ];
}
...
export async function migrateFromElectronConfig(configStore: ConfigStore): Promise<void> {
  try {
    ...
    if (Object.keys(sourceData).length === 0) {
      console.warn('[Forjinn-Desk] Config migration: source file appears empty or corrupted, will retry next startup');
      return;
    }
    ...
    console.log('[Forjinn-Desk] Config migrated from Electron desktop config:', sourcePath);
  } catch (error) {
    console.warn('[Forjinn-Desk] Config migration from Electron failed:', error);
  }
}
 ...
 * @param sourcePath - absolute path to an forjinn-desk-config.txt file
...
export async function importConfigFromFile(
  ...
  try {
    ...
    if (!path.isAbsolute(sourcePath)) {
      ...
      console.warn('[Forjinn-Desk] IMPORT_CONFIG_FROM: relative path provided, resolving to:', resolved);
      sourcePath = resolved;
    }

    const sourceData = decodeConfigFile(sourcePath);
    if (Object.keys(sourceData).length === 0) {
      console.warn('[Forjinn-Desk] IMPORT_CONFIG_FROM: file is missing, empty, or corrupted:', sourcePath);
      return;
    }
    ...
    console.log('[Forjinn-Desk] Config imported from:', sourcePath, '(overwrite:', overwrite, ')');
  } catch (error) {
    console.warn('[Forjinn-Desk] IMPORT_CONFIG_FROM failed:', error);
  }
}
