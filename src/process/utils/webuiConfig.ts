...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.FORJINN_DESK_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.FORJINN_DESK_ALLOW_REMOTE || process.env.FORJINN_DESK_REMOTE);
  const hostHint = process.env.FORJINN_DESK_HOST?.trim();
