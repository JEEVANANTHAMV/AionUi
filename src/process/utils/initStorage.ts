...
const STORAGE_PATH = {
  config: 'forjinn-desk-config.txt',
  chatMessage: 'forjinn-desk-chat-message.txt',
  chat: 'forjinn-desk-chat.txt',
  env: '.forjinn-desk-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[Forjinn-Desk] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[Forjinn-Desk] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[Forjinn-Desk] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('forjinn-desk.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'forjinn-desk-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'forjinn-desk-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'forjinn-desk-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'forjinn-desk-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  const resolveBuiltinDir = (dirPath: string): string => {
    ...
    console.warn(`[Forjinn-Desk] Could not find builtin ${dirPath} directory, tried:`, candidates);
    return candidates[0];
  };
  ...
  if (existsSync(builtinSkillsDir)) {
    try {
      ...
      console.warn(`[Forjinn-Desk] Failed to sync builtin skills directory:`, error);
    }
  }
      ...
      for (const [locale, ruleFile] of Object.entries(preset.ruleFiles)) {
        try {
          ...
          if (!existsSync(sourceRulesPath)) {
            console.warn(`[Forjinn-Desk] Source rule file not found: ${sourceRulesPath}`);
            continue;
          }
          ...
          console.warn(`[Forjinn-Desk] Failed to copy rule file ${ruleFile}:`, error);
        }
      }
    } else {
      ...
      for (const [locale, skillFile] of Object.entries(preset.skillFiles)) {
        try {
          ...
          if (!existsSync(sourceSkillsPath)) {
            console.warn(`[Forjinn-Desk] Source skill file not found: ${sourceSkillsPath}`);
            continue;
          }
          ...
          console.warn(`[Forjinn-Desk] Failed to copy skill file ${skillFile}:`, error);
        }
      }
    } else {
...
const getBuiltinAssistants = (): AcpBackendConfig[] => {
  ...
  for (const preset of ASSISTANT_PRESETS) {
    ...
    assistants.push({
      ...
      isPreset: true,
      isBuiltin: true,
      presetAgentType: preset.presetAgentType || 'forjinnrs',
...
const ensureBuiltinMcpServers = async (): Promise<void> => {
  try {
    ...
    const buildEnvFromConfig = (cfg: typeof oldConfig): Record<string, string> => {
      ...
      if (cfg.platform) env.FORJINN_DESK_IMG_PLATFORM = cfg.platform;
      if (cfg.baseUrl) env.FORJINN_DESK_IMG_BASE_URL = cfg.baseUrl;
      if (cfg.apiKey) env.FORJINN_DESK_IMG_API_KEY = cfg.apiKey;
      if (cfg.useModel) env.FORJINN_DESK_IMG_MODEL = cfg.useModel;
      return env;
    };
    ...
    if (changed) {
      ...
      console.log('[Forjinn-Desk] Built-in MCP servers ensured');
    }
    ...
    console.error('[Forjinn-Desk] Failed to ensure built-in MCP servers:', error);
  }
};
...
const cleanupOrphanedHealthCheckConversations = async () => {
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[Forjinn-Desk] Cleaned up ${deletedCount} orphaned health-check conversation(s) on startup`);
    }
  } catch (error) {
    console.warn('[Forjinn-Desk] Failed to cleanup orphaned health-check conversations:', error);
  }
};

const initStorage = async () => {
  const t0 = performance.now();
  const mark = (label: string) => console.log(`[Forjinn-Desk:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[Forjinn-Desk] Failed to initialize default MCP servers:', error);
  }
  ...
  try {
    ...
    if (!existingModels || !Array.isArray(existingModels) || existingModels.length === 0) {
      const innosynthProvider = {
        ...
        platform: 'custom',
        name: 'Innosynth',
        baseUrl: process.env.OPENAI_BASE_URL || 'http://101.53.140.44:8001/v1',
  ...
  try {
    ...
    console.error('[Forjinn-Desk] Failed to initialize builtin assistants:', error);
  }
...
export const loadSkillsContent = async (enabledSkills: string[]): Promise<string> => {
  ...
  for (const skillName of enabledSkills) {
    ...
    try {
      ...
      console.warn(`[Forjinn-Desk] Failed to load skill ${skillName}:`, error);
    }
  }
