...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'forjinn-desk');
};
 ...
 * Release builds use ~/.forjinn-desk; dev builds use ~/.forjinn-desk-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.forjinn-desk，Dev 模式使用 ~/.forjinn-desk-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'forjinn-desk');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.forjinn-desk'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.forjinn-desk-config; dev builds use ~/.forjinn-desk-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.forjinn-desk-config，Dev 模式使用 ~/.forjinn-desk-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.forjinn-desk-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[Forjinn-Desk] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[Forjinn-Desk] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[Forjinn-Desk] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[Forjinn-Desk] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
