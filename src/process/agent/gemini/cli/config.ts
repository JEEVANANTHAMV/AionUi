...
export async function loadCliConfig({
  ...
  const builtinSkillsExtension: GeminiCLIExtension = {
    name: 'forjinn-desk-builtin-skills',
    version: '1.0.0',
    isActive: true,
    path: skillsDir || '',
    contextFiles: [],
    id: 'forjinn-desk-builtin-skills',
  ...
  const config = new Config({
    ...
    // Forjinn-Desk 是桌面应用，支持用户交互确认，需要设置 interactive: true
    // Forjinn-Desk is a desktop app with user interaction support, needs interactive: true
    interactive: true,
