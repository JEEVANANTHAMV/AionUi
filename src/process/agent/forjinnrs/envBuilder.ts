...
type ForjinnrsProvider = 'anthropic' | 'openai' | 'bedrock' | 'vertex';

/**
 * Map Forjinn-Desk platform name to forjinnrs provider name.
 *
 * Forjinn-Desk PlatformType values: 'custom' | 'new-api' | 'gemini' | 'gemini-vertex-ai' | 'anthropic' | 'bedrock'
 */
function mapProvider(model: TProviderWithModel): ForjinnrsProvider {
  ...
  const mapping: Record<string, ForjinnrsProvider> = {
    anthropic: 'anthropic',
    bedrock: 'bedrock',
 ...
 * forjinnrs appends `/v1/chat/completions` internally, so passing a URL
...
/**
 * Build CLI args and env vars for spawning forjinnrs.
 */
export function buildSpawnConfig(
  ...
  // forjinnrs reads: --api-key / API_KEY / OPENAI_API_KEY / ANTHROPIC_API_KEY
  //               --base-url / BASE_URL (NOT OPENAI_BASE_URL)
  // forjinnrs appends `/v1/chat/completions` to base_url, so URLs that already
...
/**
 * Build `.forjinnrs.toml` project config content for provider compat overrides.
 ...
 */
function buildProjectConfig(model: TProviderWithModel, provider: ForjinnrsProvider): string {
