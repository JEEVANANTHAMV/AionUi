...
import { resolveForjinnrsBinary } from './binaryResolver';
import { buildSpawnConfig } from './envBuilder';
import type { ForjinnrsEvent, ForjinnrsCommand, ForjinnrsCapabilities } from './protocol';

const FORJINNRS_PROJECT_CONFIG = '.forjinnrs.toml';

type StreamEventHandler = (event: { type: string; data: unknown; msg_id: string }) => void;

/**
 * A stdio-transport MCP server to inject into the forjinnrs session. Each entry
...
export type ForjinnrsAgentOptions = {
  workspace: string;
  ...
  resume?: string;
  /**
   * Stdio MCP servers to register with the forjinnrs session after start.
   * Caller decides which MCPs belong here (team coordination, team-guide,
   * future project MCPs, etc.) — ForjinnrsAgent just forwards them.
   */
  stdioMcpServers?: StdioMcpOption[];
  onStreamEvent: StreamEventHandler;
};

export class ForjinnrsAgent {
  ...
  private options: ForjinnrsAgentOptions;
  ...
  public capabilities?: ForjinnrsCapabilities;

  constructor(options: ForjinnrsAgentOptions) {
  ...
  async start(): Promise<void> {
    ...
    if (!binaryPath) {
      throw new Error('forjinnrs binary not found');
    }
    ...
    // Write temporary .forjinnrs.toml for provider compat overrides
    if (projectConfig) {
    ...
    rl.on('line', (line) => {
      try {
        const event = JSON.parse(line) as ForjinnrsEvent;
        this.handleEvent(event);
      } catch {
        console.error('[ForjinnrsAgent] Failed to parse event:', line);
      }
    });

    // Log stderr as diagnostics
    this.childProcess.stderr?.on('data', (chunk: Buffer) => {
      console.error('[forjinnrs]', chunk.toString());
    });
    ...
    this.childProcess.on('exit', (code) => {
      ...
      if (!this.ready) {
        this.readyReject(new Error(`forjinnrs exited with code ${code} during init`));
      }
    ...
    const timeout = new Promise<void>((_, reject) => {
      setTimeout(() => reject(new Error('forjinnrs ready timeout (30s)')), 30000);
    });
    ...
    try {
      ...
      if (this.options.resume) {
        console.error('[ForjinnrsAgent] Resume failed, falling back to new session:', err);
    ...
    if (awaitAnyReady) {
      await Promise.race([
        ...
        console.warn('[ForjinnrsAgent] MCP setup warning:', err);
      });
    }
  ...
  private handleEvent(event: ForjinnrsEvent): void {
    switch (event.type) {
      case 'ready':
        this.ready = true;
  ...
  /**
   * Map forjinnrs tool_request to Forjinn-Desk confirmation details format.
   */
  private mapConfirmationDetails(event: ForjinnrsEvent & { type: 'tool_request' }) {
    const { tool } = event;
  ...
  sendCommand(cmd: ForjinnrsCommand): void {
  ...
  /**
   * Write a temporary .forjinnrs.toml in the workspace for provider compat overrides.
   * Backs up existing file content so it can be restored on exit.
   */
  private writeProjectConfig(content: string): void {
    const configPath = join(this.options.workspace, FORJINNRS_PROJECT_CONFIG);
  ...
  /**
   * Restore or remove the .forjinnrs.toml written by writeProjectConfig.
   */
