...
import { useForjinnrsMessage } from './useForjinnrsMessage';
import type { ForjinnrsModelSelection } from './useForjinnrsModelSelection';

const useForjinnrsSendBoxDraft = getSendBoxDraftHook('forjinnrs', {
  _type: 'forjinnrs',
  atPath: [],
  content: '',
  uploadFile: [],
...
const useSendBoxDraft = (conversation_id: string) => {
  const { data, mutate } = useForjinnrsSendBoxDraft(conversation_id);
...
const ForjinnrsSendBox: React.FC<{
  conversation_id: string;
  modelSelection: ForjinnrsModelSelection;
  teamId?: string;
  agentSlotId?: string;
  sessionMode?: string;
  ...
  const { thought, running, hasHydratedRunningState, tokenUsage, setActiveMsgId, setWaitingResponse, resetState } =
    useForjinnrsMessage(conversation_id, {
      onConfigChanged: (capabilities) => {
        const modes = (capabilities as { modes?: string[] })?.modes;
        if (modes && modes.length > 0) {
          setDynamicModes(mergeWithCapabilities('forjinnrs', modes));
        }
      },
    });
    ...
    async ({ input, files }: Pick<ConversationCommandQueueItem, 'input' | 'files'>) => {
      ...
      try {
        ...
        if (teamId) {
          ...
          assertBridgeSuccess(result, 'Failed to send message to Forjinn CLI');
        }
        emitter.emit('chat.history.refresh');
        if (files.length > 0) {
          emitter.emit('forjinnrs.workspace.refresh');
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const storageKey = `forjinnrs_initial_message_${conversation_id}`;
    const processedKey = `forjinnrs_initial_processed_${conversation_id}`;
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        console.error('[ForjinnrsSendBox] Failed to send initial message:', error);
  ...
  const onSendHandler = async (message: string) => {
    ...
    clearFiles();
    emitter.emit('forjinnrs.selected.file.clear');
  ...
  useAddEventListener('forjinnrs.selected.file', setAtPath);
  useAddEventListener('forjinnrs.selected.file.append', (selectedItems: Array<string | FileOrFolderItem>) => {
    ...
    <div className='max-w-800px w-full mx-auto flex flex-col mt-auto mb-16px'>
      ...
      <SendBox
        ...
        onSelectedWorkspaceItemsChange={(items) => {
          emitter.emit('forjinnrs.selected.file', items);
          setAtPath(items);
        }}
        loading={isBusy}
                    ...
                    return (
                      <Tag
                        ...
                        onClose={() => {
                          ...
                          emitter.emit('forjinnrs.selected.file', newAtPath);
                          setAtPath(newAtPath);
                        }}
                      >
                        {item.name}
...
export default ForjinnrsSendBox;
