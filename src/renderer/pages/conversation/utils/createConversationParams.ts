...
async function resolvePreferredMode(backend: string): Promise<string | undefined> {
  ...
  if (backend === 'gemini') {
    ...
    preference = await ConfigStorage.get('forjinnrs.config');
  } else {
...
/**
 * Get a model from configured providers that is compatible with forjinnrs.
 * forjinnrs supports all platforms via OpenAI-compatible protocol.
 * Throws if no compatible provider is configured.
 */
export async function getDefaultForjinnrsModel(): Promise<TProviderWithModel> {
...
export async function buildCliAgentParams(
  ...
  } else if (type === 'forjinnrs') {
    // Forjinnrs needs a real model from configured providers (anthropic, openai, ali-intl, aws)
    model = await getDefaultForjinnrsModel();
  } else {
