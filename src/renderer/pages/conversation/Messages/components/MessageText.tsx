...
const parseFileMarker = (content: string) => {
  const markerIndex = content.indexOf(FORJINN_DESK_FILES_MARKER);
  if (markerIndex === -1) {
    return { text: content, files: [] as string[] };
  }
  const text = content.slice(0, markerIndex).trimEnd();
  const afterMarker = content.slice(markerIndex + FORJINN_DESK_FILES_MARKER.length).trim();
  ...
  return (
    <>
      <div className={classNames('min-w-0 flex flex-col group', isUserMessage ? 'items-end' : 'items-start')}>
        ...
        <div
          className={classNames('min-w-0 [&>p:first-child]:mt-0px [&>p:last-child]:mb-0px max-w-full', {
            'bg-forjinn-2 p-8px': isUserMessage || cronMeta,
