...
type ProviderAgentKey = 'gemini' | 'forjinnrs';

/** Map agent key → storage key for persisting default model */
const MODEL_STORAGE_KEY: Record<ProviderAgentKey, 'gemini.defaultModel' | 'forjinnrs.defaultModel'> = {
  gemini: 'gemini.defaultModel',
  forjinnrs: 'forjinnrs.defaultModel',
};
...
export const useGuidModelSelection = (agentKey: ProviderAgentKey = 'forjinnrs'): GuidModelSelectionResult => {
  ...
  const modelList = useMemo(() => {
    ...
    // 'gemini'. Other provider-based agents (e.g. forjinnrs) do not support
