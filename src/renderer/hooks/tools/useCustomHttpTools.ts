...
export const useCustomHttpTools = () => {
  ...
  const saveTools = useCallback(
    (toolsOrUpdater: ICustomHttpTool[] | ((prev: ICustomHttpTool[]) => ICustomHttpTool[])) => {
  ...
  const addTool = useCallback(
    (toolData: Omit<ICustomHttpTool, 'id' | 'createdAt' | 'updatedAt'>) => {
      const now = Date.now();
  ...
  const updateTool = useCallback(
    (id: string, toolData: Partial<ICustomHttpTool>) => {
      return saveTools((prev) => prev.map((t) => (t.id === id ? { ...t, ...toolData, updatedAt: Date.now() } : t)));
    },
    [saveTools]
  );
