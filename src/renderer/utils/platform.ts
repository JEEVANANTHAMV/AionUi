...
const ASSET_PROTOCOL_PREFIX = 'forjinn-asset://asset/';
 ...
 * - In Electron dev / any HTTP(S)-served renderer: keep `forjinn-asset://` because direct `file://` is blocked.
 * - In Electron packaged / local-protocol renderers: convert `forjinn-asset://asset/{path}` to `file://` for reliable image loading.
 * - In a regular browser (WebUI): convert `forjinn-asset://asset/{path}` to `/api/ext-asset?path={encodedPath}`.
 *
