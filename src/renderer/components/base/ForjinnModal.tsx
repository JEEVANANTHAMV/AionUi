...
/** ForjinnModal 组件 Props */
export interface ForjinnModalProps extends Omit<ModalProps, 'title' | 'footer'> {
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <ForjinnModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </ForjinnModal>
 *
 * // 预设尺寸 / Preset size
 * <ForjinnModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </ForjinnModal>
 *
...
const ForjinnModal: React.FC<ForjinnModalProps> = ({
  children,
  size,
  header,
  footer,
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={`forjinn-desk-modal ${className}`}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div className='forjinn-desk-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='forjinn-desk-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
...
ForjinnModal.displayName = 'ForjinnModal';

export default ForjinnModal;
