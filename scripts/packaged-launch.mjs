...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('Forjinn-Desk.exe');
    await killProcessByName('Forjinn-Desk');
  ...
  const env = {
    ...process.env,
    FORJINN_DESK_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] FORJINN_DESK_EXTENSIONS_PATH: ${env.FORJINN_DESK_EXTENSIONS_PATH}`);
