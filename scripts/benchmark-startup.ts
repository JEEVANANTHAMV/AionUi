 ...
 * parsing the electron-log file for [Forjinn-Desk:ready] / [Forjinn-Desk:init] /
 * [Forjinn-Desk:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [Forjinn-Desk:ready] marks
  ...
  // Parsed from [Forjinn-Desk:init] marks
  initTotalMs: number;
  // Parsed from [Forjinn-Desk:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'Forjinn-Desk-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'Forjinn-Desk', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'Forjinn-Desk', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'Forjinn-Desk', 'logs', `${today}.log`));
  }
...
// Matches: [Forjinn-Desk:ready] <label> +<ms>ms
// Matches: [Forjinn-Desk:init]  <label> +<ms>ms
// Matches: [Forjinn-Desk:process] <label> +<ms>ms
const MARK_REGEX = /\[Forjinn-Desk:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[Forjinn-Desk] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[Forjinn-Desk] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[Forjinn-Desk] Showing main window')) marks.logs.showingMainWindow = true;
  }
