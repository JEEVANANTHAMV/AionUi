...
**Worktree path:** always `/tmp/forjinn-desk-verify-<PR_NUMBER>` — never use `/tmp/forjinn-desk-pr-*` (reserved for pr-automation/pr-fix).
...
WORKTREE_DIR="/tmp/forjinn-desk-verify-${PR_NUMBER}"
...
Create worktree at `/tmp/forjinn-desk-verify-<PR_NUMBER>` (reuse if already created in Step 2):
...
```bash
for dir in /tmp/forjinn-desk-verify-*; do
...
```
 ...
 4. Impact analysis + test supplementation:
    a. Create worktree at /tmp/forjinn-desk-verify-<PR> (reuse if from Step 2)
 ...
 7. Cleanup all /tmp/forjinn-desk-verify-* worktrees → session summary (merged/auto-merge/rejected/skipped)
```
