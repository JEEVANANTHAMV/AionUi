...
```
~/.forjinn-desk-fix-issues/skip-list.json
```
...
```bash
gh api repos/iOfficeAI/Forjinn-Desk/issues/<number>/comments \
...
```bash
gh issue list --repo iOfficeAI/Forjinn-Desk --state open --label bug --limit 50 \
...
gh api repos/iOfficeAI/Forjinn-Desk/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo iOfficeAI/Forjinn-Desk --body "$(cat <<'EOF'
...
```bash
gh pr list --repo iOfficeAI/Forjinn-Desk --state open --search "<error-keyword-or-file>" --json number,title
```
...
In daemon mode (`limit > 0`), after the summary report, update `~/.forjinn-desk-fix-issues/skip-list.json`
...
| --------- | ---------------- | ------------------------------------------------------------------- |
| repo      | iOfficeAI/Forjinn-Desk | GitHub repository (owner/repo)                                      |
