...
WORKTREE_DIR="/tmp/forjinn-desk-ship-${PR_NUMBER}"
...
- **Worktree path** — always `/tmp/forjinn-desk-ship-<PR_NUMBER>` (distinct from pr-automation's `/tmp/forjinn-desk-pr-*` and pr-verify's `/tmp/forjinn-desk-verify-*`)
