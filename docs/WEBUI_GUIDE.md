# Forjinn-Desk WebUI Mode - Startup Guide

Forjinn-Desk supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts Forjinn-Desk with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use Forjinn-Desk from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\Forjinn-Desk\Forjinn-Desk.exe" --webui

# Or if Forjinn-Desk is in your PATH
Forjinn-Desk.exe --webui
```
...
```bash
# Using full path
/Applications/Forjinn-Desk.app/Contents/MacOS/Forjinn-Desk --webui

# Or using open command
open -a Forjinn-Desk --args --webui
```

### Method 2: Create Shell Script

Create `start-forjinn-desk-webui.sh`:
...
```bash
chmod +x start-forjinn-desk-webui.sh
./start-forjinn-desk-webui.sh
```
...
# Or using full path
/opt/Forjinn-Desk/forjinn-desk --webui
```
...
chmod +x Forjinn-Desk-*.AppImage

# Run with --webui flag
./Forjinn-Desk-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/forjinn-desk-webui.desktop`:

```ini
[Desktop Entry]
Name=Forjinn-Desk WebUI
Comment=Start Forjinn-Desk in WebUI mode
Exec=/opt/Forjinn-Desk/forjinn-desk --webui
Icon=forjinn-desk
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/forjinn-desk-webui.desktop
```
...
Create `~/bin/start-forjinn-desk-webui.sh`:
...
```bash
chmod +x ~/bin/start-forjinn-desk-webui.sh
```
...
Create `/etc/systemd/system/forjinn-desk-webui.service`:

```ini
[Unit]
Description=Forjinn-Desk WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/Forjinn-Desk/forjinn-desk --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable forjinn-desk-webui.service
sudo systemctl start forjinn-desk-webui.service

# Check status
sudo systemctl status forjinn-desk-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run Forjinn-Desk in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making Forjinn-Desk accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running Forjinn-Desk WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/Forjinn/Forjinn-Desk/issues/217)
...
#### 4. Download and Install Forjinn-Desk

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/Forjinn/Forjinn-Desk/releases
wget https://github.com/Forjinn/Forjinn-Desk/releases/download/vVERSION/Forjinn-Desk_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/Forjinn/Forjinn-Desk/releases/download/vVERSION/Forjinn-Desk_VERSION_arm64.deb

# Install the package
apt install -y ./Forjinn-Desk_*.deb

# Verify installation
which Forjinn-Desk
```

#### 5. Launch Forjinn-Desk WebUI

```bash
# Start Forjinn-Desk in WebUI mode with no-sandbox flag
Forjinn-Desk --no-sandbox --webui
```
...
To access Forjinn-Desk from other devices on your local network:

```bash
# Start with --remote flag
Forjinn-Desk --no-sandbox --webui --remote
...
Forjinn-Desk --no-sandbox --webui --port 8080
```
...
chmod +x /opt/Forjinn-Desk/forjinn-desk
```

#### Out of Memory

Forjinn-Desk requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if Forjinn-Desk is running: look for "Server started" message
...
4. **Keep device charged** - Running Forjinn-Desk consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **Forjinn-Desk Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch Forjinn-Desk quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-forjinn-desk.sh << 'EOF'
#!/bin/bash
echo "Starting Forjinn-Desk WebUI..."
Forjinn-Desk --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-forjinn-desk.sh

# Run anytime
./start-forjinn-desk.sh
```
...
```bash
proot-distro login ubuntu -- bash -c "Forjinn-Desk --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/Forjinn/Forjinn-Desk/issues/217)
...
```cmd
Forjinn-Desk.exe --webui --remote
```

### macOS

```bash
/Applications/Forjinn-Desk.app/Contents/MacOS/Forjinn-Desk --webui --remote
```

### Linux

```bash
forjinn-desk --webui --remote
```
...
netsh advfirewall firewall add rule name="Forjinn-Desk WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add Forjinn-Desk
...
```cmd
where Forjinn-Desk.exe
```

**macOS:**

```bash
mdfind -name "Forjinn-Desk.app"
```
...
which forjinn-desk
# or
find /opt -name "forjinn-desk" 2>/dev/null
```
...
```powershell
& "C:\Program Files\Forjinn-Desk\Forjinn-Desk.exe" --webui 2>&1 | Tee-Object -FilePath forjinn-desk.log
```

**macOS/Linux:**

```bash
/path/to/forjinn-desk --webui 2>&1 | tee forjinn-desk.log
```
...
export FORJINN_DESK_PORT=8080

# Allow remote access without passing --remote
export FORJINN_DESK_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as FORJINN_DESK_ALLOW_REMOTE=true)
export FORJINN_DESK_HOST=0.0.0.0

# Then start the application
forjinn-desk --webui

# You can also pass the port directly via CLI
forjinn-desk --webui --port 8080
```
...
```cmd
# Using full path
"C:\Program Files\Forjinn-Desk\Forjinn-Desk.exe" --resetpass

# Or for a specific user
"C:\Program Files\Forjinn-Desk\Forjinn-Desk.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/Forjinn-Desk.app/Contents/MacOS/Forjinn-Desk --resetpass

# Or for a specific user
/Applications/Forjinn-Desk.app/Contents/MacOS/Forjinn-Desk --resetpass username
```
...
```bash
# Using system path
forjinn-desk --resetpass

# Or for a specific user
forjinn-desk --resetpass username

# Or using full path
/opt/Forjinn-Desk/forjinn-desk --resetpass
```
...
- [GitHub Issues](https://github.com/Forjinn/Forjinn-Desk/issues)
...
2. Search [existing issues](https://github.com/Forjinn/Forjinn-Desk/issues)
3. Create a [new issue](https://github.com/Forjinn/Forjinn-Desk/issues/new) with:
   - Your OS and version
   - Forjinn-Desk version
   - Steps to reproduce
   - Error messages or logs

---

**Happy using Forjinn-Desk in WebUI mode!** 🚀
