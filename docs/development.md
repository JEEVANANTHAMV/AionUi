...
git clone https://github.com/Forjinn/Forjinn-Desk.git
cd Forjinn-Desk
...
When you have two clones of the repository (e.g. `Forjinn-Desk` and `Forjinn-Desk-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `FORJINN_DESK_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`Forjinn-Desk-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.forjinn-desk-dev-2`, `~/.forjinn-desk-config-dev-2`)
...
Forjinn-Desk uses **electron-vite** for fast bundling:
