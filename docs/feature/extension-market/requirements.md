...
由 CI 从 `Forjinn/AionHub` 仓库自动生成，客户端拉取此文件获取可用 extension 列表。
...
```json
{
  ...
  "extensions": {
    "aionext-auggie": {
      ...
      "version": "1.0.0",
      "description": "Integrates Augment Code as an ACP adapter in Forjinn-Desk.",
    ...
    "aionext-claude": {
      ...
      "version": "1.0.0",
      "description": "Integrates Anthropic's Claude Code CLI as an ACP adapter in Forjinn-Desk.",
      "author": "Aionui Official",
      "engines": {
        "forjinn-desk": "^1.0.0"
      },
  ...
  "metadata": {
    ...
    "repository": "https://github.com/Forjinn/AionHub/"
  }
}
```
...
`engines.forjinn-desk` 定义了兼容的 AionUI 版本范围，
...
判断 Hub extension 的展示状态:
  ...
  extension 目录不存在 (~/.forjinn-desk/extensions/aionext-xxx/)
...
APP 启动
  │
  ├── 1. ExtensionLoader 扫描 ~/.forjinn-desk/extensions/ (现有逻辑)
  │      → 得到已安装 extension 列表
  │
...
用户点击 Install
  ...
  │
  ├── 4. 解压到临时目录
  │      └── ~/.forjinn-desk/extensions/.tmp/aionext-xxx/
  │
  ├── 5. 校验 aion-extension.json 合法性
  │      └── 失败 → 清理临时目录, 状态 → install_failed
  │
  ├── 6. 原子移动到正式目录
  │      └── .tmp/aionext-xxx/ → ~/.forjinn-desk/extensions/aionext-xxx/
  │
...
用户点击 Uninstall (更多菜单中)
  ...
  │      └── 失败 → 仅 warn 日志, 不阻塞卸载
  │
  ├── 5. 删除 ~/.forjinn-desk/extensions/aionext-xxx/ 目录
  │
...
用户点击 Retry (在 install_failed 状态)
  │
  ├── 检查 ~/.forjinn-desk/extensions/aionext-xxx/ 是否已存在
...
> (拆另外仓库, 比如 `Forjinn/AionHub`，专门存放 Hub 相关的 extension 和构建脚本)
...
└── hub/
    ├── index.json                   ← 构建 APP 时从 forjinn-desk/hub 的 dist/ 复制
...
拉取 remote-index.json
  ├── 优先: GitHub Raw
  │   https://raw.githubusercontent.com/forjinn-desk/hub/main/dist/index.json
  │
  ├── 备选: jsDelivr CDN
  │   https://cdn.jsdelivr.net/gh/forjinn-desk/hub@main/dist/index.json
  │
  ├── 超时: 5s (不阻塞 APP 启动)
  ├── 重试: 1 次
  │
...
for (const hubAgent of hubAgents) {
  ...
  const extDir = `~/.forjinn-desk/extensions/${hubAgent.name}/`;
