...
# 版本定义在每个 extension 的 aion-extension.json 中
{
  ...
  "engines": { "forjinn-desk": ">=2.0.0" }  ← 兼容性声明
}
```
...
APP 启动 / 定期 (每 24h)
  ...
  │       AND semver.satisfies(appVersion, remote.engines.forjinn-desk)
  │       → 标记为 "可更新"
  │
  └── UI 展示 Update 按钮
```
...
- 后续可加: 在安装新版前备份旧版到 `~/.forjinn-desk/extensions/.backup/`, 支持一键回退

**版本号规范**:

- 严格 semver (`major.minor.patch`)
- CI 校验: manifest 中的 `version` 必须大于上一次发布版本 (防止降版本覆盖)
- `engines.forjinn-desk` 使用 semver range (如 `>=2.0.0`, `^2.1.0`)
...
async function verifyExtension(tgzBuffer: Buffer, bundleJson: string): Promise<boolean> {
  ...
  await verify(bundle, tgzBuffer, {
    ...
    certificateIdentityRegExp: /https:\/\/github\.com\/forjinn-desk\/hub\/.*/,
  });
...
const trustedIdentities = [
  /https:\/\/github\.com\/forjinn-desk\/hub\/.*/, // 官方仓库
...
客户端下载 extension
  ...
  │   └── 版本匹配 → 直接使用 (零网络)
  │
  ├── 2. 检查本地缓存 (~/.forjinn-desk/cache/)
  │   └── 版本匹配 + integrity 验证 → 直接使用
  │
  ├── 3. 从 GitHub 下载 (主源)
  │   ├── 优先: jsDelivr CDN (国内有节点)
  │   │   https://cdn.jsdelivr.net/gh/forjinn-desk/hub@main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   ├── 备选: GitHub Raw
  │   │   https://raw.githubusercontent.com/forjinn-desk/hub/main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   └── 备选: GitHub Releases (如果迁移到 Releases)
  │       https://github.com/forjinn-desk/hub/releases/download/v2026.03/ext-claude-code-1.2.0.tgz
  │
  ├── 4. 下载后
  │   ├── 验证 SHA-512 integrity
  │   ├── 验证 Sigstore 签名 (Phase 2)
  │   ├── 缓存到 ~/.forjinn-desk/cache/
  │   └── 解压到 ~/.forjinn-desk/extensions/ext-claude-code/
  │
  └── 5. 所有源失败
      └── 显示错误提示, 建议检查网络
```
...
下载 .tgz 完成
  ...
  │
  ├── 3. 解压到临时目录
  │   └── ~/.forjinn-desk/extensions/.tmp/ext-claude-code/
  │
  ├── 4. 二次校验
...
```mermaid
flowchart TD
  subgraph GitHub["GitHub (forjinn-desk/hub 仓库)"]
    direction TB
    ExtDirs["extensions/\next-claude-code/\next-opencode/\n..."]
  ...
  subgraph App["AionUI APP"]
    ...
    ExtDir["~/.forjinn-desk/extensions/\n已安装 extension"]
