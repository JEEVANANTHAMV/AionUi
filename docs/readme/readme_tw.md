...
**支援的 Agent：** 內建 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Aion CLI（forjinnrs，Forjinn-Desk 自帶的 Rust 後端服務） • Snow CLI • Kiro • Hermes Agent • Cursor Agent 等
...
- **支援的後端** — Claude Code、Codex、Gemini、Snow CLI、Aion CLI（forjinnrs）；其他具備 `mcpCapabilities.stdio` 的 ACP 後端自動支援
...
| AWS Bedrock 憑證                         | 透過 Aion CLI（forjinnrs）的 Bedrock 驅動 Agent |
