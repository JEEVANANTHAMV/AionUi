...
**支持的 Agent：** 内置 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Aion CLI（forjinnrs，Forjinn-Desk 自带的 Rust 后端服务） • Snow CLI • Kiro • Hermes Agent • Cursor Agent 等
...
- **支持的后端** — Claude Code、Codex、Gemini、Snow CLI、Aion CLI（forjinnrs）；其他具备 `mcpCapabilities.stdio` 的 ACP 后端自动支持
...
| AWS Bedrock 凭证                         | 通过 Aion CLI（forjinnrs）的 Bedrock 驱动 Agent |
