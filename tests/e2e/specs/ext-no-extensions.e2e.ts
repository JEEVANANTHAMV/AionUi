...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forjinn-desk-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forjinn-desk-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      FORJINN_DESK_EXTENSIONS_PATH: emptyExtensionsDir,
      FORJINN_DESK_EXTENSION_STATES_FILE: extensionStatesFile,
