/**
 * GAP-3: ForjinnrsManager Finish Fallback — Black-box & boundary tests
 *
 * Tests based on GAP-3-plan.md acceptance criteria.
 * Validates that ForjinnrsManager synthesizes a finish signal when
...
function createManager(conversationId = 'conv-fb-1'): ForjinnrsManager {
  const data = {
...
describe('GAP-3: ForjinnrsManager Finish Fallback Mechanism', () => {
  ...
  describe('AC-5: mainWarn logged on fallback', () => {
    it('logs a warning with conversation_id when fallback fires', () => {
      ...
      expect(mockMainWarn).toHaveBeenCalledWith('[ForjinnrsManager]', expect.stringContaining('conv-fb-1'));
    });
  });
