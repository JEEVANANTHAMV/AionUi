    ...
    const servers: IMcpServer[] = [
      {
        ...
        transport: {
          ...
          env: {
            FORJINN_DESK_IMG_PLATFORM: 'openai',
            FORJINN_DESK_IMG_MODEL: 'gpt-image-1',
          },
        },
        createdAt: 1,
        updatedAt: 1,
    ...
    expect(result).toEqual([
      {
        ...
        args: ['/abs/builtin-mcp-image-gen.js'],
        env: [
          { name: 'FORJINN_DESK_IMG_PLATFORM', value: 'openai' },
          { name: 'FORJINN_DESK_IMG_MODEL', value: 'gpt-image-1' },
        ],
      },
      {
        type: 'http',
...
const mockUnrelatedMcpAgents = (emptyDetect: () => Promise<IMcpServer[]>) => {
  ...
  vi.doMock('../../src/process/services/mcpServices/agents/ForjinnrsMcpAgent', () => ({
    ForjinnrsMcpAgent: makeAgentClass(emptyDetect),
  }));
};
