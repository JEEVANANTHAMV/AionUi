/**
 * GAP-4: ForjinnrsManager Cron Command Feedback Loop — Black-box tests
 *
...
vi.mock('@process/agent/forjinnrs', () => ({
  ForjinnrsAgent: vi.fn().mockImplementation(() => ({
...
import { ForjinnrsManager } from '@/process/task/ForjinnrsManager';

// ── Helpers ─────────────────────────────────────────────────────────

function createManager(): ForjinnrsManager {
  const data = {
  ...
  return new ForjinnrsManager(data as any, model);
}

/** Simulate a complete turn: start → content deltas → finish */
function simulateTurn(manager: ForjinnrsManager, textChunks: string[], msgId = 'msg-1') {
  (manager as any).emit('forjinnrs.message', { type: 'start', data: '', msg_id: msgId });
  for (const chunk of textChunks) {
    (manager as any).emit('forjinnrs.message', { type: 'content', data: chunk, msg_id: msgId });
  }
  (manager as any).emit('forjinnrs.message', { type: 'finish', data: '', msg_id: msgId });
}

// ── Tests ───────────────────────────────────────────────────────────

describe('GAP-4: ForjinnrsManager Cron Command Feedback Loop', () => {
  let manager: ForjinnrsManager;
...
[/CRON_CREATE]`;
  ...
  describe('AC-4: sends feedback to forjinnrs agent', () => {
...
[CRON_LIST]`;
    ...
    it('handles empty content on finish (no start event)', async () => {
      ...
      (manager as any).emit('forjinnrs.message', { type: 'finish', data: '', msg_id: 'msg-empty' });
      await new Promise((r) => setTimeout(r, 50));
      expect(mockCronService.listJobsByConversation).not.toHaveBeenCalled();
    });

    it('handles non-string content data gracefully', async () => {
      (manager as any).emit('forjinnrs.message', { type: 'start', data: '', msg_id: 'msg-x' });
      // Non-string data should be ignored (not accumulated)
      (manager as any).emit('forjinnrs.message', { type: 'content', data: 12345, msg_id: 'msg-x' });
      (manager as any).emit('forjinnrs.message', { type: 'finish', data: '', msg_id: 'msg-x' });
      await new Promise((r) => setTimeout(r, 50));
      expect(mockCronService.listJobsByConversation).not.toHaveBeenCalled();
    });
