 ...
 *
 * Tests the enable/disable flow for the forjinn-desk-skills builtin skill:
...
const BUNDLED_SKILL_PATH = path.resolve(
  __dirname,
  '../../src/process/resources/skills/_builtin/forjinn-desk-skills/SKILL.md'
);

// Mock Electron app and initStorage before importing AcpSkillManager
vi.mock('electron', () => ({ app: { setName: vi.fn(), getPath: () => '/tmp/forjinn-desk-test' } }));
vi.mock('../../src/process/utils/initStorage', () => ({
  getSkillsDir: () => path.join('/tmp/forjinn-desk-test', 'skills'),
  getAutoSkillsDir: () => path.join('/tmp/forjinn-desk-test', 'skills', '_builtin'),
  getBuiltinSkillsCopyDir: () => path.join('/tmp/forjinn-desk-test', 'builtin-skills'),
}));
...
describe('Skills Market - Bundled SKILL.md', () => {
  ...
  it('has valid frontmatter with name and description', async () => {
    ...
    expect(nameMatch![1].trim()).toBe('forjinn-desk-skills');
  ...
  it('contains the curl command for fetching full SKILL.md', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('curl -s https://skills.forjinn-desk.com/SKILL.md');
  });
  ...
  it('references the standard credentials path', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('~/.config/forjinn-desk-skills');
  });
...
describe('Skills Market - Enable/Disable flow', () => {
  ...
  it('enable: creates forjinn-desk-skills directory with SKILL.md', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'forjinn-desk-skills');
    ...
    expect(written).toContain('name: forjinn-desk-skills');
  });

  it('disable: removes forjinn-desk-skills directory completely', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'forjinn-desk-skills');
  ...
  it('disable: fs.rm with force does not throw if directory does not exist', async () => {
    const skillDir = path.join(tmpDir, '_builtin', 'forjinn-desk-skills');
