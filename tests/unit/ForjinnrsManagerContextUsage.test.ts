/**
 * GAP-2: ForjinnrsManager Context Usage Persistence — Black-box tests
 *
 * Tests based on GAP-2-plan.md acceptance criteria.
 * Validates that ForjinnrsManager persists token usage from stream_end
...
const {
  ...
  mockDb: {
    ...
    getConversation: vi.fn(() => ({ success: true, data: { type: 'forjinnrs', extra: {} } })),
...
function createManager(conversationId = CONV_ID): ForjinnrsManager {
  const data = {
    workspace: '/test/workspace',
    model: { name: 'test-provider', useModel: 'test-model', baseUrl: '', platform: 'test' },
    conversation_id: conversationId,
  };
  return new ForjinnrsManager(data as any, data.model as any);
}

function emitEvent(manager: ForjinnrsManager, event: Record<string, unknown>) {
  (manager as any).emit('forjinnrs.message', event);
}

// ── Tests ──────────────────────────────────────────────────────────

describe('GAP-2: ForjinnrsManager Context Usage Persistence', () => {
  ...
  beforeEach(() => {
    ...
    mockDb.getConversation.mockReturnValue({
      success: true,
      data: { type: 'forjinnrs', extra: { workspace: '/test' } },
    });
