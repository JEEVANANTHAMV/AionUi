...
import { FORJINN_DESK_STRICT_ENV_ENV } from '../../../src/process/extensions/constants';
import {
...
describe('extensions/envResolver', () => {
  ...
  it('全局严格模式应缓存并可通过 clearStrictModeCache 失效', () => {
    process.env[FORJINN_DESK_STRICT_ENV_ENV] = '1';
    clearStrictModeCache();

    expect(isGlobalStrictMode()).toBe(true);

    process.env[FORJINN_DESK_STRICT_ENV_ENV] = '0';
    // 仍使用缓存值
    expect(isGlobalStrictMode()).toBe(true);
