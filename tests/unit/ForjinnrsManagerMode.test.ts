/**
 * ForjinnrsManager mode-based auto-approval — unit tests
 *
...
function createManager(sessionMode: string): ForjinnrsManager {
  const data = {
    workspace: '/test',
...
describe('ForjinnrsManager.tryAutoApprove', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
...
describe('ForjinnrsManager.setMode', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  it('should send set_mode command to forjinnrs agent', async () => {
